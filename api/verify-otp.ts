...
// When deployed to Vercel, this will automatically have process.env available
// For local development, ensure SUPABASE_SERVICE_ROLE_KEY is in your .env file

// @ts-ignore - process is available in serverless environment
...
export default async function handler(
  ...
  try {
    ...
    if (existingUser) {
      ...
      console.log('[OTP Verify API] Generating session for existing user...');
      const { data: tokenData, error: tokenError } = await supabase.auth.admin.generateLink({
        type: 'magiclink',
        email: email,
        options: {
          redirectTo: `${process.env.VERCEL_URL ? 'https://' + process.env.VERCEL_URL : 'http://localhost:5173'}/profile-setup`
        }
      });

      if (tokenError) {
        console.error('[OTP Verify API] Failed to generate token:', tokenError.message);
        return res.status(500).json({ error: 'Failed to create session' });
      }

      // For existing users, we'll use the magic link approach
      // But we need to create a proper session instead
      // Since we can't directly create sessions without password, we return user data for now
      session = {
        user: {
          id: userId,
          email: email,
        ...
        access_token: null, // Will be generated on client side
      };
